...
let dotDpsTotal = 0;
    ...
    const globalStatusBuff = headDotBuff + sBonus.dot;

    let dotBreakdown = { 
        nativeTick: 0, radTick: 0, nativeDps: 0, radDps: 0,
        relicMult: relicDotMult, critMult: dotCritMult,
        hasNative: uStats.dot > 0,
        hasRad: traitObj.hasRadiation,
        // Compatibility fields for the renderer:
        base: (uStats.dot || 0) + (traitObj.dotBuff || 0) + globalStatusBuff,
        baseNoHead: (uStats.dot || 0) + (traitObj.dotBuff || 0) + sBonus.dot,
        finalTick: 0, timeUsed: 0
    };

    if (dotBreakdown.hasNative) {
        const nativeTickPct = (uStats.dot + globalStatusBuff) * relicDotMult;
        const totalNativeDmg = finalDmg * (nativeTickPct / 100) * dotCritMult;
        const nativeTimeBasis = Math.max(uStats.dotDuration || 0, finalSpa);
        const nativeUnitDps = totalNativeDmg / nativeTimeBasis;
        const canStackNative = traitObj.allowDotStack || traitObj.allowPlacementStack;
        dotBreakdown.nativeDps = nativeUnitDps * (canStackNative ? placement : 1);
        dotBreakdown.nativeTick = totalNativeDmg;
    }

    if (dotBreakdown.hasRad) {
        const radTickPct = (traitObj.dotBuff + globalStatusBuff) * relicDotMult;
        const totalRadDmg = finalDmg * (radTickPct / 100) * dotCritMult;
        const radTimeBasis = Math.max(traitObj.radiationDuration || 0, finalSpa);
        const radUnitDps = totalRadDmg / radTimeBasis;
        const canStackRad = traitObj.allowDotStack || traitObj.allowPlacementStack;
        dotBreakdown.radDps = radUnitDps * (canStackRad ? placement : 1);
        dotBreakdown.radTick = totalRadDmg;
    }

    dotDpsTotal = dotBreakdown.nativeDps + dotBreakdown.radDps;
    dotBreakdown.finalTick = dotBreakdown.nativeTick + dotBreakdown.radTick;
