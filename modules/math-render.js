...
function renderMathContent(data) {
    ...
    return `
        ...
        <div class="deep-dive-content hidden">
            ...
            ${data.dot > 0 ? `
            <div class="dd-section">
                <div class="dd-title text-accent-end"><span>${data.extraAttacks ? 5 : 4}. Status Effect (DoT)</span> <button class="calc-info-btn" onclick="openInfoPopup('dot_logic')">?</button></div>
                <table class="calc-table">
                    <tr><td class="mt-cell-label calc-sub">Base Hit Ref</td><td class="mt-cell-val calc-sub">${num(data.dmgVal)}</td></tr>
                    <tr><td class="mt-cell-label">Base Tick %</td><td class="mt-cell-val">${fix(data.dotData.baseNoHead, 2)}%</td></tr>
                    ${headDotRow}
                    <tr><td class="mt-cell-label text-bold">Total Tick %</td><td class="mt-cell-val text-bold">${fix(data.dotData.base, 2)}%</td></tr>
                    <tr><td class="mt-cell-label">Relic Mult (x${fix(data.dotData.relicMult, 2)})</td><td class="mt-cell-val text-accent-end">${fix(data.dotData.finalPct, 1)}%</td></tr>
                    <tr><td class="mt-cell-label text-custom">Crit Avg Mult</td><td class="mt-cell-val text-custom">x${fix(data.dotData.critMult, 3)}</td></tr>
                    <tr><td class="mt-cell-label">Total Damage (Lifetime)</td><td class="mt-cell-val">${num(data.dotData.finalTick)}</td></tr>
                    <tr><td class="mt-cell-label calc-sub">Time Basis</td><td class="mt-cell-val calc-sub">${fix(data.dotData.timeUsed, 2)}s</td></tr>
                    <tr><td class="mt-cell-label text-white">DoT DPS (1 Unit)</td><td class="mt-cell-val text-accent-end">${num(data.singleUnitDoT)}</td></tr>
