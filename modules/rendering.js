...
function generateBuildRowHTML(r, i, unitConfig = {}) {
    ...
    const s = r.subStats;
    const subInnerHtml = (s.head || s.body || s.legs) 
        ? `${s.head ? `<div class="stat-line"><span class="sl-label">SUB</span> ${getRichBadgeHtml(s.head)}</div>` : ''}
           ${s.body ? `<div class="stat-line"><span class="sl-label">BODY</span> ${getRichBadgeHtml(s.body)}</div>` : ''}
           ${s.legs ? `<div class="stat-line"><span class="sl-label">LEGS</span> ${getRichBadgeHtml(s.legs)}</div>` : ''}`
    ...
    return `
        <div class="build-row ${rankClass} ${sortMode === 'efficiency' ? 'is-efficiency-sort' : ''}">
            <div class="br-header">
                ...
                <span class="prio-badge ${pCfg.cls}">${pCfg.label}</span>
            </div>
            <div class="br-grid">
...
function updateBuildListDisplay(unitId) {
    ...
    const renderList = (builds) => {
        ...
        let filtered = builds.filter(r => {
            ...
            return searchText.includes(searchInput) && (prioSelect === 'all' || r.prio === prioSelect) && 
